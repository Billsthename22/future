...
import { useRef, Suspense, use, useState } from "react";
...
export default function ShowroomPage({ params }: { params: Promise<{ brand: string }> }) {
  ...
  const cars = displayCars.filter((car) => car.brandId === brand);
  const [currentIndex, setCurrentIndex] = useState(0);

  const safeCars = cars.length > 0 ? cars : displayCars;
  const activeCar = safeCars[currentIndex % safeCars.length];

  const nextCar = () => setCurrentIndex((prev) => (prev + 1) % safeCars.length);
  const prevCar = () => setCurrentIndex((prev) => (prev - 1 + safeCars.length) % safeCars.length);
      ...
      <div className="absolute inset-0 z-10">
        <Canvas shadows camera={{ position: [0, 2, 12], fov: 35 }}>
          <Suspense fallback={<HologramLoader />}>
            <Stage environment="night" intensity={0.5} contactShadow={{ opacity: 0.8, blur: 3 }}>
              <Car key={activeCar.id} brand={brand} modelId={activeCar.id} />
            </Stage>
          </Suspense>
          <OrbitControls 
            enableZoom={false} 
      ...
      <div className="relative z-30 p-8 md:p-12 h-full flex flex-col justify-between pointer-events-none">
        ...
        <div className="flex justify-between items-start">
          <motion.div initial={{ x: -30, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
            ...
            <h1 className={`${orbitron.className} text-7xl md:text-[10rem] font-black uppercase italic tracking-tighter leading-[0.8] text-transparent stroke-text drop-shadow-2xl`}>
              {brand}
            </h1>
            <style jsx>{`
              .stroke-text {
          ...
          <div className="text-right flex flex-col items-end gap-2">
            <div className="px-4 py-2 border border-white/5 backdrop-blur-xl bg-white/[0.02] shadow-2xl relative group">
               ...
               <p className="text-[9px] text-white/40 uppercase tracking-tighter font-bold">Data_Node</p>
               <p className="text-xs font-black uppercase tracking-widest text-white italic">LK_HQ // 26.03</p>
            </div>
          </div>
        </div>
        ...
        <div className="absolute top-1/2 left-0 w-full -translate-y-1/2 flex justify-between px-8 pointer-events-none">
          ...
          >
            <div className="w-6 h-6 flex items-center justify-center text-white/40 group-hover:text-red-500 transition-colors group-hover:-translate-x-1 transition-transform">←</div>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-16 relative z-10">
                <div className="space-y-1">
                  ...
                  <p className={`${orbitron.className} text-4xl font-black uppercase tracking-tighter leading-none`}>{activeCar.name}</p>
                </div>
                ...
                <div className="md:px-12 md:border-x border-white/10 space-y-1">
                  ...
                  <div className="w-full h-[3px] bg-white/5 mt-4 overflow-hidden rounded-full">
                    <motion.div 
                      ...
                      style={{ width: `${(activeCar.hp / 700) * 100}%` }} 
